function switchTab(tab) {
  document.getElementById('prod').style.display = (tab === 'prod') ? 'block' : 'none';
  document.getElementById('retest').style.display = (tab === 'retest') ? 'block' : 'none';
  const tabs = document.querySelectorAll('.tab');
  tabs.forEach(el => el.classList.remove('active'));
  if (tab === 'prod') tabs[0].classList.add('active');
