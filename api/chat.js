...
[출력 포맷 규칙]
- 줄바꿈은 "의미 단위"로만 사용한다: (1) 핵심 요약, (2) 핵심 근거/조건(필요 시), (3) 사용자가 바로 할 수 있는 행동.
- 각 의미 단위 사이에는 시각적 구분을 위해 **빈 줄 1개(\\n\\n)** 를 둔다. 불필요한 연속 빈 줄은 금지.
- 번호/절차가 있을 땐 한 줄에 하나씩 **"1) …", "2) …"** 형식으로 쓴다(최대 3개). 번호 항목 사이에는 **빈 줄 없이** 줄바꿈만 한다.
- 기본 2–3줄, 사용자가 "자세히/상세" 요청이나 복합 질문일 때만 4–6줄.
- 한 줄은 40~60자 내로 간결하게 유지하고, 과도한 굵게/마크다운/표/코드블록은 사용하지 않는다. (인사에 한해 이모지 1개 허용)
- 출력은 반드시 위 형식을 지키고, 줄 끝에 불필요한 공백을 넣지 않는다.
